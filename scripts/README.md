...
- **추가 생성물**: `results/sewer_infrastructure_map.html` (인터랙티브 지도)

### 5. `create_sewer_infrastructure_notebook.py` (신규)
- **파일 크기**: 28KB (518 lines)
- **생성일**: 2025-07-30
- **최종 수정**: 2025-07-30 (인터랙티브 지도 시각화 추가)
...
2. **분석 코드 포함**
   - 전처리된 데이터 로드
   - 하수도 인프라 지수 계산 (가중 평균 방식)
   - 다양한 시각화 (히스토그램, 파이차트, 상관관계 히트맵 등)
   - 상위/하위 지역 분석
   - 시도별 분석
   - **인터랙티브 지도 시각화** (Folium 기반 Choropleth 지도)
   - 결과 저장
...
4. **한글 폰트 설정**
   - 나눔고딕 폰트 적용으로 한글 시각화 최적화
   - matplotlib 폰트 설정 자동화
